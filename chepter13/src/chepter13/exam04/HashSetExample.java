...
// 중복된 데이터 제거가 필요할떄 유용(예: ArrayList 의 요소에서 중복을 제거 할 때)
public class HashSetExample {
	...
	public static void main(String[] args) {
		...
		Iterator<String> iterator = set.iterator(); // 반복자 얻기(Iterator 구현객체를 반환)
		while (iterator.hasNext()) {// 가져올 객체가 있으면 true, 없으면 false
			...
			if ("Java".equals(element)) {
				iterator.remove();
				// 참고 : remove() 는 next() 호출 이휴에만 호출 가능하며, 한 번만 가능
			} 

		}System.out.println(set);
		// 겍체 삭제 
		// index 를 가지고 있는 List와 다르게 index가 없음
		// 객체의 hashCode()와 equals()를 기준으로 판단
		
		set.remove("JDBC"); // 1객의 객체 삭제
		set.remove("iBatis");
		
		System.out.println("총객체의 수: " + set.size());
		
		// 향상 된 for 문으로 모든 객체를 하나씩 가져오기
		// 인덱스가 없으므로 일단 for 문 사용 불가
...
//		System.out.println("교집합: " + class1);
		...
		// 차집합
		// 기준 HashSet에서 대상 HashSet 과의 교집합 제외
		...
		//요소 정렬
		// HashSet 자체를 정렬 기능이 없는 컬렉션이기 때문에,
		// 정렬을 하려면 다른 컬렉션으로 변환한 뒤 정렬
		// List 로 변환후 Collections.sort() 사용
...
//		Collections.sort(fruits);  // 불가 List 계열만 가능
		...
		// Set -> List 로 변환
		List<String> fruitsLinst = new ArrayList<String>(fruits);
		
		// 정렬
		Collections.sort(fruitsLinst); // 오름차순 정렬
		System.out.println(fruitsLinst);
		
		// 필요하면 정렬된 순서를 유지하는 Set 으로 다시 변환 
		Set<String> sotedSet = new LinkedHashSet<String>(fruitsLinst);
		System.out.println(sotedSet);
