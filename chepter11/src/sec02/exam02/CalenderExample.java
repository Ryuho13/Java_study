...
public class CalenderExample {
	...
	public static void main(String[] args) {
		Calendar now = Calendar.getInstance(); // Calendar 의 하위 (자식) 객체를 리턴
		
		int year = now.get(Calendar.YEAR); // 연도
//		int year = now.get(1); // 위에상 같은 상수
		...
		if(hour12 == 0) hour12 = 12; // 24시간제 기준으로 0시, 12 시 일때 -> 12시
		...
		if (today.isBefore(tagetDate)) {
			...
			System.out.println("오늘은 크리스마스 이후입나다.");
		}else {
		...
		// Duration 클래스 : 두 시간 사이의 간격을 다루느 클래스
