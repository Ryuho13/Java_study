...
public class RandomExample {
	...
	public static void main(String[] args) {
		// Random 클래스 
		// Math.random() 보다 더 다양한 기능화 제어를 제공
		// 정수, 실수, 논리값 등 다양한 타입의 난수 생성
		 Random random = new Random();
		 
		 // 아래 주석 해제한 뒤 여러번 실해해 보 것
//		 random.setSeed(1234);
		 // 시드(seed)란?
		 // 난수 생성기의 "초기값"
		 // 동일한 시드를 넣으면 항상 같은 난수 순서(시퀸스)가 생성됨
		 // 시드를 지정하지 않으면 시스템의 현재 시각넹 따라 자동으로 지정됨
		...
		boolean randBollean1 = random.nextBoolean();
		boolean randBollean2 = random.nextBoolean();
		
		System.out.println(randBollean1);
		System.out.println(randBollean2);
