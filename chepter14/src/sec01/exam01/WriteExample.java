...
//		char c = '다';
		// 해결 방법 2가지 
		// 1. 문자 기반 FileWriter 사용(OutputStream 사용) 
		...
		os.close(); // 출력 스트림이 사용했던 자원들(메모리)를 풀어주기위해
		// 참고: close() 안에서도 스트림을 닫기 전에 flush() 기능을 수행함
		
		// 중요: try- with - resources
		// 리소스를 자동으로 닫아주는  try 문법
		// close() 를 명시적으로 호출하지 않아도, try블록이 끓기거나 예와가 발생하는 즉시 자동으로 정리
		
		try (OutputStream os2 =  new FileOutputStream("C:/Temp/test.db")){
			os2.write(a);
			os2.write(b);
			os2.write(c);
		} catch (Exception e) {
